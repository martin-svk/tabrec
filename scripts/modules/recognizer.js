...
(function() {
  ...
  this.Recognizer = (function() {
    var _accuracy, _dbg_mode, _last_event_time, _last_pattern_name, _last_pattern_time, _max_running_average_bucket_size, _max_running_average_event_gap, _min_running_average_event_gap, _notifier, _pattern_recognizers, _rec_timeout, _running_average_bucket, _running_average_gap_inclusion_threshold, get_current_ts, get_running_average, handle_running_average, has_suffix, in_threshold, inside_running_average, is_bottom_outlier, is_upper_outlier, not_inside_timeout, record_event, reset_all_pattern_states, same_pattern_again, some_pattern_occured, tab_activated, tab_attached, tab_created, tab_detached, tab_moved, tab_removed, tab_updated, update_current_sequences;
    ...
    some_pattern_occured = function() {
      var j, len, pattern;
      for (j = 0, len = _pattern_recognizers.length; j < len; j++) {
        pattern = _pattern_recognizers[j];
    ...
    update_current_sequences = function(event_name, event_data) {
      var j, len, pattern, results;
      results = [];
      for (j = 0, len = _pattern_recognizers.length; j < len; j++) {
        pattern = _pattern_recognizers[j];
        results.push(pattern.register_event(event_name, event_data));
      }
      return results;
    };
    ...
    reset_all_pattern_states = function() {
      ...
      for (j = 0, len = _pattern_recognizers.length; j < len; j++) {
        pattern = _pattern_recognizers[j];
        results.push(pattern.reset_states());
      }
      return results;
    };
